...
public class ProyectoServiceImpl implements ProyectoService {
	...
	public ProyectoServiceImpl(ProyectoRepository proyectoRepository,
			ProyectoEstudianteRepository proyectoEstudianteRepository, ProyectoMapper proyectoMapper) {
	...
	@Override
	public PageDtoWrapper<Proyecto, ProyectoCreationDTO.ProyectoDTO> findProyectosByEstudiante(int page, int size,
			String carnet, int status) {
		Pageable paging = this.getPageable(page, size);
	...
	public ProyectoCreationDTO.ProyectoDTO save(ProyectoCreationDTO proyecto) {
		try {
			...
			var proyectoEstudianteList = proyecto.getEstudiantes().stream()
					...
					.map(estudiante -> new ProyectoEstudiante(estudiante, savedProyecto)).collect(Collectors.toList());
	...
	public ProyectoDTO update(ProyectoCreationDTO proyecto, int idProyecto) {
		try {
			var proyectoDB = this.proyectoRepository.findById(idProyecto).map(obj -> {
				obj.setDuracion(proyecto.getDuracion());
				obj.setInterno(proyecto.isInterno());
				obj.setNombre(proyecto.getNombre());
				return obj;
			}).orElseThrow(() -> new ResourceNotFoundException(
					String.format("Project with id %d does not exist", idProyecto)));
